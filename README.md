...
1. **Spring Boot `#49582` 머지**
   ...
   - 공통 로직을 바꾸지 않고 `spring-boot-mail` 모듈 안에서 전용 `FailureAnalyzer`와 targeted test로 해결했습니다.

2. **Spring Data Redis `#3361` 리뷰 대기**
   - 중복 converter 구현을 deprecated API 유지 + 내부 위임 구조로 정리했습니다.
   - public API를 바로 삭제하지 않고 호환성을 고려한 리팩터링으로 접근했습니다.

3. **종료된 PR 12개 회고**
   - assignee, triage 상태, maintainer 코멘트, 기존 PR 확인을 readiness gate로 만들었습니다.
   - 실패 기록을 숨기지 않고 다음 PR의 작업 기준으로 바꿨습니다.
